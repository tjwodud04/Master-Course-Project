...
# engine.setProperty('voice', voices[1].id)  # us female
...
def parse_day_subtree(current_state, old_state):
    ...
    elif 30 > int(float(current_state['Sanity'])) > SANITY_DANGER:
        status['Sanity'].append(int(float(current_state['Sanity'])))

        action = 'Quite_crushed'
        utterance = '정신력 회복이 필요해요!'  # ko
    ...
    if current_state['Curr_Equip_Hands'] != 'nil':
        ...
        # ko
        utterance = f"{str(current_state['Curr_Equip_Hands'][9:])}를 장비하고 있어요!"
    ...
    if current_state['Attack_Target'] != 'nil':
        ...
        action = 'Attacked'
        utterance = f"{str(current_state['Attack_Target'][9:])}을 공격했어요!"  # ko
    ...
    if current_state['Defense_Target'] != 'nil':
        ...
        # ko
        utterance = f"{str(current_state['Defense_Target'][9:])}에 공격 받았어요!"
    ...
    if current_state['Tool'] == 'Less resources':
        ...
        utterance = '도구를 만들기 위해서 자원이 더 필요해요!'  # ko
        ...
        utterance = '도끼와 곡괭이를 만들 수 있어요!'  # ko
    ...
    if current_state['Lights'] == 'Less resources' and current_state['Is_Light'] == 'No lights!':
        ...
        action = 'resources_are_sufficient_to_make_fire_fit'
        utterance = '화덕을 만들 수 있어요!'  # ko
